...
local batt_s = 'VFAS'
  ...
  if batt_t > 3 then
    ...
    cell = batt_t / batt_c
    if cell < 3.7 then
          lcd.drawText(4, 55, round(batt_t,1)..'v', BLINK)
          lcd.drawText(lcd.getLastPos() + spacing, 55, round(cell, 1) .. 'v', BLINK)
    else 
          lcd.drawText(4, 55, round(batt_t,1)..'v', LEFT)
  ...
  drawCurrentTime(onehalf_column - 10, first_line -5)
  ...
  lcd.drawText(182, 55, getValue('RSSI')..'db', SMLSIZE)
